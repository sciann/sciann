...
def test_variable():
    x = sn.Variable('x', 10, dtype='float32')
    assert sn.is_variable(x)
    assert sn.is_functional(x)
...
def test_functional(variable_x):
    x = variable_x
    f = sn.Functional('f', x, 2*[10], 'tanh')
    assert sn.is_functional(f)
...
def test_functional_exceptions(variable_x):
    ...
    with pytest.raises(TypeError):
        f = sn.Functional(x)
    with pytest.raises(TypeError):
        f = sn.Functional('f', 2*[10])
    with pytest.raises(TypeError):
        f = sn.Functional('f', x, 'tanh')
    with pytest.raises(TypeError):
        f = sn.Functional('f', x, 2*[10], 12)
